...
public class VariableEx1 {
	...
	public static void main(String[] args) {
		 ...
		 * [정수형]
		 * 기본적으로 리터럴이 int로 설정
		 * int(4byte) 안에서 int보다 byte 크기가 작은 short나 byte로 리터럴 변경할 경우 
		 ...
		 * int는 위에서 작성한 숫자 범위를 초과하기 때문에 초과하는 숫자를 담기 위해서는 뒤에 L이나 ㅣ
		 * 
		 * long 변수명 = long
		 ...
		 * 왜냐하면 float은 대갹 소수점아래 7번째자리까지만 표기 가능
		 ...
		 * double 1번 = 3.14;
		 * 
		 * float 플롯1번 = 3.145F; //대소문자 상관없이 f를 붙여줌
		 * 플롯1번의 공간의 크기를 foat으로 주고 실수 뒤에 f를 붙여주지 않으면
		 * 실수는 기본리터럴 형인 dobule로 인식이 되어
		 * 공간보다 큰 메모리가 온다고 컴퓨터는 인식을 하고 오류를 발생하기 때문에
		 * 앞에 예약어를 float로 작성했다면 소수 뒤에는 f를 붙여주는 것
		 * 
		 * L이나 f같은 리터럴 표기법은
		 ...
		 */
			...
			float 소수1번 = 3.14f;//Type mismatch: cannot overseen
